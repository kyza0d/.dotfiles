        ...
        vim.api.nvim_create_autocmd('FileType', {
          ...
          callback = function(args)
            vim.schedule(function()
              ...
              -- Skip if buffer/window is invalid
              if not vim.api.nvim_buf_is_valid(bufnr) or win == -1 then return end

              -- Get buffer and window info for context checking
              local buf_name = vim.api.nvim_buf_get_name(bufnr)
              local buf_type = vim.api.nvim_get_option_value('buftype', { buf = bufnr })
              local win_config = vim.api.nvim_win_get_config(win)

              -- Define conditions to skip applying settings
              local should_skip = false

              -- Skip for floating windows (documentation, completion, etc.)
              if win_config.relative and win_config.relative ~= '' then should_skip = true end

              -- Skip for specific buffer types that are temporary/special
              if buf_type == 'nofile' or buf_type == 'quickfix' or buf_type == 'help' then
                -- Exception: still apply settings for 'qf' filetype on quickfix buffers
                if not (ft == 'qf' and buf_type == 'quickfix') then should_skip = true end
              end

              -- Skip for unnamed buffers that might be temporary
              if buf_name == '' and buf_type == 'nofile' then should_skip = true end

              -- Allow override via buffer variable for manual control
              local force_apply = vim.b[bufnr].settings_force_apply
              local force_skip = vim.b[bufnr].settings_force_skip
              ...
              -- Skip if conditions indicate this is a temporary/floating context
...
-- Helper functions for manual control when needed
_G.settings_force_apply = function(bufnr)
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  vim.b[bufnr].settings_force_apply = true
  vim.b[bufnr].settings_force_skip = nil
end

_G.settings_force_skip = function(bufnr)
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  vim.b[bufnr].settings_force_skip = true
  vim.b[bufnr].settings_force_apply = nil
end

_G.settings_reset_overrides = function(bufnr)
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  vim.b[bufnr].settings_force_apply = nil
  vim.b[bufnr].settings_force_skip = nil
end
