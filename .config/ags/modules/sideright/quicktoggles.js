  ...
  return Widget.Button({
    ...
    onClicked: (self) => {
      ...
      if (self.attribute.enabled) {
        Utils.execAsync('hyprsunset -t 2500').catch(print);
      } else {
        Utils.execAsync('pkill hyprsunset').catch(print);
      }
    },
...
export const ModuleGrayscale = async (props = {}) => {
  if (!exec(`bash -c 'command -v hyprshade'`)) return null;
  return Widget.Button({
    attribute: { enabled: false },
    className: 'txt-small sidebar-iconbutton',
    ...
    onClicked: async (self) => {
      try {
        const currentShader = exec('hyprshade current').trim();
        const isGrayscaleActive = currentShader === 'grayscale';

        if (isGrayscaleActive) {
          await Utils.execAsync('hyprshade off');
          self.attribute.enabled = false;
        } else {
          await Utils.execAsync('hyprshade on grayscale');
          self.attribute.enabled = true;
        }
        self.toggleClassName('sidebar-button-active', self.attribute.enabled);
      } catch (error) {
        print(`Error toggling grayscale: ${error}`);
      }
    },
    child: MaterialIcon('invert_colors', 'norm'),
    setup: (self) => {
      setupCursorHover(self);
      self.poll(1000, (button) => {
        const currentShader = exec('hyprshade current').trim();
        const isGrayscaleActive = currentShader === 'grayscale';
        button.attribute.enabled = isGrayscaleActive;
        button.toggleClassName('sidebar-button-active', isGrayscaleActive);
      });
    },
    ...props,
  });
