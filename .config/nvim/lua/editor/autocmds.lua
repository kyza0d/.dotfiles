...
-- Colorscheme caching system
local colorscheme_cache_file = vim.fn.stdpath('cache') .. '/colorscheme.txt'

local function save_colorscheme()
  local current_colorscheme = vim.g.colors_name
  if current_colorscheme then
    local file = io.open(colorscheme_cache_file, 'w')
    if file then
...
local function load_cached_colorscheme()
  local file = io.open(colorscheme_cache_file, 'r')
  if file then
    local cached_colorscheme = file:read('*all'):gsub('%s+', '')
    file:close()
    if cached_colorscheme ~= '' then ky.pcall('Failed to load cached colorscheme', vim.cmd.colorscheme, cached_colorscheme) end
  end
end

-- Load cached colorscheme on startup
create_autocmd('VimEnter', {
  callback = load_cached_colorscheme,
  group = group,
})

-- Save colorscheme when it changes and refresh indentblankline highlights
